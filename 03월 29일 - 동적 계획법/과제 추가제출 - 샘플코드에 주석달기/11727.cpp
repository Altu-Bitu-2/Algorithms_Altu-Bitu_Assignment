...
int fillTile(int n) {
    vector<int> dp(n + 1, 0); // 0번부터 n번까지 0으로 초기화. 인덱스 0번은 비워두고 1부터 값 업데이트 예정 
    ...
    dp[1] = 1; // 너비 1짜리 경우는 1가지 
    dp[2] = 3; // 너비 2짜리 경우는 3가지 
    for (int i = 3; i <= n; i++) {
        dp[i] = dp[i - 1] + 2 * dp[i - 2]; // 너비 2짜리 경우 * 직전 경우의 수 + 너비 1짜리 경우 * 전전 경우의 수 
        dp[i] %= MOD; //구하는 과정에서 int 범위 초과할 수 있으므로 마지막에 한 번이 아니라 중간 중간 모듈러 연산을 해줘야 함
    }
    return dp[n]; // n까지의 모든 경우의 수 구한 후 return 
}
 ...
 * 이때, 중복 경우의 수가 생기지 않도록 너비 2의 경우에서 1에서 더한 경우는 빼줌 -> 3 - 1 = 2 경우 
