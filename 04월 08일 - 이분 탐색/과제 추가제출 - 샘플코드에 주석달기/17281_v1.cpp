...
int hitScore(int hit, vector<bool> &baseman) {
    int score = 0;
    for (int i = 3; i >= 0; i--) { // 1루~홈 
        if (!baseman[i]) { // 선수가 없다면 
            continue; // 다음 
        }
        if (i + hit >= 4) { // 선수가 있고 쳤다면 
            score ++; // 점수 늘려주기 
        } else {
            baseman[i + hit] = true; // 선수 있음 
        }
        baseman[i] = false; 
    }
    if (hit == 4) { 
    ...
    for (int i = 0; i < n; i++) { //i: 이닝
        ...
        while (out < 3) { // 3진 아웃이 되기 전까지 
            ...
            if (state == 0) { // 아웃 
                ...
                score += hitScore(state, baseman); // 아웃이 아니면 점수가 남 
            }
        }
    }
    return score;
