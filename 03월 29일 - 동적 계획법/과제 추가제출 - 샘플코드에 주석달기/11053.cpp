...
int lis(int n, vector<int> &arr) {
    vector<int> dp(n, 1); // 증가하는 부분 수열 길이의 최댓값을 저장할 dp 
    int ans = 1;

    for (int i = 1; i < n; i++) {
        for (int j = 0; j < i; j++) {
            if (arr[i] > arr[j]) { //증가하는 관계라면
                dp[i] = max(dp[i], dp[j] + 1); // 기존 값과 이전값에 +1 한 것 중 더 큰 것 저장 
            }
        }
