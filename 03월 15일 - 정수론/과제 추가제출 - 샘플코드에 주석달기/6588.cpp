...
vector<bool> isPrime() {
    ...
    is_prime[0] = is_prime[1] = false; //0과 1은 당연히 소수가 아니므로 false 
   	// i가 소수인지 판단 
    for (int i = 2; i * i <= SIZE; i++) { 
        ...
        //i가 소수라면
        for (int j = i * i; j <= SIZE; j += i) { // i*i부터 시작하는 이유는 i-1까지는 앞에서 이미 지워졌기 때문 
            is_prime[j] = false; //i의 배수 제거. 어떤 수의 배수가 되는 수는 당연히 소수가 아니다. 
        }
    }
    return is_prime; //소수이면 1 return, 아니면 0 return  
}
...
int main() {
    ...
    int n;
    vector<bool> is_prime = isPrime(); //소수 판단 여부 미리 저장

    while (true) { //무한루프 
        //입력
        cin >> n; //확인할 짝수 정수 n 입력받기 
        if (n == 0) //입력에 0이 주어지면 break 
            break; 

        //연산
        int prime = checkGoldbach(n, is_prime); //골드바흐의 추측이 맞는지 체크
        if (prime) //맞다면 -> 두 홀수 소수 중 작은 값 리턴받은 경우
            cout << n << " = " << prime << " + " << n - prime << '\n'; //n - prime은 두 홀수 소수 중 큰 값 
        else //틀리다면 -> 0 리턴받은 경우
