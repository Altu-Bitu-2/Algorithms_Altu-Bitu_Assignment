...
stack<int> calcPlus(string &a, string &b) {
    ...
    while (p1 >= 0 && p2 >= 0) { //자릿수 더하기
        int num = (a[p1--] - '0') + (b[p2--] - '0'); //a와 b의 각 자릿수 값 더하고 앞자리로 가기 위해 감소 연산해주기 
        num += carry; // 뒷자리에서 발생했던 carry 값과 함께 더해주기 
        carry = num / 10;  //자릿수 더한 값이 10을 넘으면 올림변수 체크
        st.push(num % 10); //올림값 제외한 값(더한 각 자리수)만 스택에 저장
    }
    ...
    if (carry) { //마지막 올림 확인
        st.push(1); // 올림값이 있다면 자릿수가 늘어나며 1이 생긴 것이므로 추가로 1 넣어주기 
    }
...
int main() {
    ...
    if (a.length() < b.length()) { //b가 더 길다면
        swap(a, b); // a, b 바꿔주기. A > B 라고 가정했기 때문 
    }
    st = calcPlus(a, b); 
