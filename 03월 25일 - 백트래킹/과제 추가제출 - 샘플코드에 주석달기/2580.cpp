...
bool fillSudoku(int idx) {
    ...
    for (int i = 1; i <= MAX; i++) { //1~9까지 넣어보기
        if (!check_row[row][i] && !check_col[col][i] && !check_3x3[(row / 3) * 3 + col / 3][i]) { // 해당 숫자가 없다면 
            check_row[row][i] = true; // 업데이트 
            check_col[col][i] = true; // 업데이트 
            check_3x3[(row / 3) * 3 + col / 3][i] = true; // 업데이트 
            sudoku[row][col] = i; // 해당 숫자 넣어주기 
            if (fillSudoku(idx + 1)) {
                return true; // 종료조건을 써주어야 함수가 종료된다 
            }
            // 원래대로 돌려주기 
...
int main() {
    ...
    for (int i = 0; i < MAX; i++) {
        for (int j = 0; j < MAX; j++) {
            cin >> sudoku[i][j]; // 빈칸이 있는 스도쿠 입력받기 
            if (sudoku[i][j]) { //스도쿠 상태 보고, check배열들 초기화
            	// 스도쿠가 빈칸(0)이 아니라면 true로 변경 
                check_row[i][sudoku[i][j]] = true; // 행 
                check_col[j][sudoku[i][j]] = true; // 열 
                check_3x3[(i / 3) * 3 + j / 3][sudoku[i][j]] = true; // 구역 
            }
        }
    }
