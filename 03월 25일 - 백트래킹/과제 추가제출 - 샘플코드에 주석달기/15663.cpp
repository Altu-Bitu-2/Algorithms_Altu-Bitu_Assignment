...
void backtracking(int cnt) {
    if (cnt == m) { //길이 m 이라면 (기저조건)
        for (int i = 0; i < m; i++) {
            cout << ans[i] << ' '; // m개의 수를 모두 고른 것이므로 출력 
        }
        cout << '\n';
        return;
    }
    ...
    for (int i = 0; i < n; i++) {
        if (!check[i] && prev != num[i]) { //수 위치 중복 체크 && 이전에 탐색한 값이 아니라면
            prev = num[i]; // prev 값 업데이트 
            ans[cnt] = num[i]; // 답에 저장 
            check[i] = true; // check 배열 업데이트해서 중복 방지 
            backtracking(cnt + 1); // 그 다음 수 고르기 
            check[i] = false; // 원래대로 돌려주기 
        }
    }
}
...
int main() {
    ...
    //연산
    sort(num.begin(), num.begin() + n); // 사전순 출력을 위해 오름차순 정렬 
