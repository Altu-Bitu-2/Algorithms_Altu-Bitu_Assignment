    ...
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < m; j++) {
            if (land[i][j] > height) { //블록 제거
                b += (land[i][j] - height); // 높이만큼 블록 제거하고 개수 업데이트 
                tot_time += 2 * (land[i][j] - height); // 제거하는데 걸리는 시간은 2초 
            } else if (land[i][j] < height) { //블록 쌓기
                b -= (height - land[i][j]); 
                tot_time += (height - land[i][j]); // 쌓는데 걸리는 시간은 1초 
            }
        }
    }
...
int main() {
    ...
    //입력
    cin >> n >> m >> b; // 집터 세로, 가로, 인벤토리에 있는 블록 개수 
    ...
    for (int i = min_height; i <= max_height; i++) { // 브루트포스 
        int t = mineLand(n, m, b, i, land);
        if (t <= min_time) { //가장 빨리 작업이 끝나는 높이
            min_time = t; // 최소시간 업데이트 
            height = i; // 해당 높이 저장 
        }
    }
